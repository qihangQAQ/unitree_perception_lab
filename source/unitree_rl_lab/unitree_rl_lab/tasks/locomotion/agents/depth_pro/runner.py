"""Runner wiring the repository-local depth-pro policy into RSL-RL."""
...
class DepthProRunner(OnPolicyRunner):
    """OnPolicyRunner that constructs the local depth policy and Old-HIM PPO."""

    def _construct_algorithm(self, obs) -> DepthProPPO:
        ...
        if policy_class_name != "DepthProActorCritic":
            raise ValueError(f"Unsupported depth-pro policy class: {policy_class_name!r}.")
        policy = DepthProActorCritic(
            obs,
            self.cfg["obs_groups"],
        ...
        if algorithm_class_name != "DepthProPPO":
            raise ValueError(f"Unsupported depth-pro algorithm class: {algorithm_class_name!r}.")
        algorithm = DepthProPPO(
            policy,
            device=self.device,
            **self.alg_cfg,
