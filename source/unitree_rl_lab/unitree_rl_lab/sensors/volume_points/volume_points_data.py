"""Data container for the volume-point sensor."""
    ...
    def make_zero(
        ...
        return VolumePointsData(
            ...
            ang_vel_w=torch.zeros(
                (num_envs, num_bodies, 3), device=device, dtype=dtype
            ),
            point_num_each_body=point_num_each_body,
            points_pos_w=torch.zeros(
                (num_envs, num_bodies, point_num_each_body, 3),
                device=device,
                dtype=dtype,
            ),
            points_vel_w=torch.zeros(
                (num_envs, num_bodies, point_num_each_body, 3),
                device=device,
                dtype=dtype,
            ),
