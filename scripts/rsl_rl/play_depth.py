...
"""Play an RSL-RL checkpoint with keyboard velocity control and per-step depth dump.
...
On top of the normal play loop, every simulation step dumps the current raw depth frame
(raycaster_camera ``distance_to_image_plane``, no noise) to a timestamped txt file under
``--dump_depth_dir``. Each run writes into its own run-timestamped subdirectory, so a later
run never deletes an earlier run's frames; within a run, only the latest frame is kept.
"""
...
parser.add_argument(
    ...
    type=str,
    default="depth_image",
    help="Directory to dump the latest raw depth frame as a timestamped txt (only the latest file is kept).",
)
...
def _make_run_dir(dump_dir: str) -> str:
    """Create a run-specific subdirectory so each run keeps its own latest frame.

    The subdirectory is named by the run's start timestamp; a numeric suffix is appended
    if it already exists, so a later run never deletes an earlier run's saved frames.
    """
    os.makedirs(dump_dir, exist_ok=True)
    base = time.strftime("%Y-%m-%d_%H-%M-%S")
    run_dir = os.path.join(dump_dir, base)
    index = 1
    while os.path.exists(run_dir):
        run_dir = os.path.join(dump_dir, f"{base}_{index}")
        index += 1
    os.makedirs(run_dir, exist_ok=True)
    return run_dir


def _save_depth_frame(env, run_dir: str) -> None:
    """Dump the current raw depth frame to a timestamped txt, keeping only the latest file.

    Reads the ``raycaster_camera`` sensor's ``distance_to_image_plane`` output directly
    (no observation noise is applied at the sensor level), so this is the cleanest
    available depth. The frame is a ``(H, W)`` matrix of distances in meters, written as
    ``<H>`` rows x ``<W>`` space-separated columns. Only ``run_dir`` is touched, so the
    deletion of the previous frame never affects other runs.
    """
    camera = env.unwrapped.scene.sensors["raycaster_camera"]
    depth = camera.data.output["distance_to_image_plane"]  # (N, H, W, 1)
    depth = depth[0, :, :, 0].cpu().numpy()  # (H, W)

    for old in glob.glob(os.path.join(run_dir, "*_sim-depth.txt")):
        os.remove(old)

    timestamp = time.strftime("%Y-%m-%d_%H-%M-%S")
    path = os.path.join(run_dir, f"{timestamp}_sim-depth.txt")
    np.savetxt(path, depth, fmt="%.6f")
...
def main():
    ...
    try:
        ...
        run_dir = _make_run_dir(args_cli.dump_depth_dir)
        print(f"[INFO] Dumping latest depth frame to: {run_dir}")
        ...
        while simulation_app.is_running():
            ...
            with torch.inference_mode():
                ...
                # Dump the latest raw depth frame after every simulation step.
                _save_depth_frame(env, run_dir)
